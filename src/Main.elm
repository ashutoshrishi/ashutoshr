...
init result =
    let
        ...
        -- Combine
        mainInit =
            Model defaultPage headerInit blogInit postInit

        -- Initial update
        ( mainModel, updateMsg ) =
            urlUpdate result mainInit
    in
        ( mainModel
...
update msg model =
    case msg of
        BlogMsg bmsg ->
            let
                ( newModel, newMsg ) =
                    Blog.update bmsg model.blogModel
            in
                ( { model | blogModel = newModel }
        ...
        HeaderMsg hmsg ->
            let
                ( newHeader, newMsg ) =
                    Header.update hmsg model.headerModel
            in
                ( { model | headerModel = newHeader }
        ...
        PostMsg pmsg ->
            let
                ( newPostComp, newMsg ) =
                    PostComp.update pmsg model.postModel
            in
                ( { model | postModel = newPostComp }
...
viewPage model =
    case model.page of
        BlogPage ->
            App.map BlogMsg (Blog.view model.blogModel)
