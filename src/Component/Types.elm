...
postDecoder =
    let
        ...
        created =
            "created" := Json.string
    in
        Json.object4 createPost title content slug created
...
renderMarkdown str cls =
    ...
    in
        if cls == "" then
            Markdown.toHtmlWith options [] str
        else
            Markdown.toHtmlWith options [ class cls ] str
...
renderPostBody isExcerpt post action =
    if String.length post.content > 500 && isExcerpt then
        let
            excerpt =
                String.left 500 post.content ++ " ..."
        in
            div []
                [ renderMarkdown excerpt "post-content"
                , p []
                    [ a
                        [ onClick (action post.slug)
